...
## 项目结构
评测目录下包含多轮评估与回归测试产物，涵盖不同运行模式与计划文档，形成可重复、可对比的性能基准体系。关键路径如下：
...
## 架构总览
性能基准测试由“计划—运行—采集—分析—报告”闭环构成。计划文档定义任务与指标，运行脚本驱动执行，日志与摘要记录结果，最后形成对比分析与改进建议。
...
Runner --> Exec["执行环境<br/>real-eval-runs / rerun / rerun-abs"]
...
Metrics --> Report["报告与对比<br/>m4-regression-eval-report.md / 最新摘要"]
...
## 结论
通过 real-eval-runs、rerun 与 abs 的组合，μ 性能基准测试实现了从端到端评估、稳定性验证到确定性校验的完整闭环。配合计划、脚本、日志与安全扫描，能够系统化地发现性能瓶颈、评估改进效果并保障质量与安全。建议持续迭代计划与脚本，扩大任务覆盖面，细化指标维度，强化自动化与可视化。
