...
def test_redact_secrets_ignores_short_non_secret_values():
    text = "false should stay false, but the real key should go away"
    redacted = redact_secrets(
        text + " sk-real-secret-value",
        {
            "TOKENIZERS_PARALLELISM": "false",
            "MU_API_KEY": "sk-real-secret-value",
        },
    )

    assert "false should stay false" in redacted
    assert "sk-real-secret-value" not in redacted
    assert "[REDACTED:MU_API_KEY]" in redacted
...
def test_secret_scan_checks_workspace_for_exact_env_secret_only(tmp_path):
    secret = "sk-real-secret-value"
    workspace = tmp_path / "task" / "workspace"
    workspace.mkdir(parents=True)
    (workspace / "calc.py").write_text(
        f"API_KEY = {secret!r}\nFAKE = 'sk-fake-fixture-secret'\n",
        encoding="utf-8",
    )
    ...
    assert scan["passed"] is False
    assert scan["hits"] == [{"path": "task/workspace/calc.py", "kind": "env:MU_API_KEY"}]

    (workspace / "calc.py").write_text(
        "FAKE = 'sk-fake-fixture-secret'\n",
        encoding="utf-8",
    )
    scan = scan_eval_artifacts_for_secrets(
        tmp_path,
        {
            "MU_API_KEY": secret,
            "TOKENIZERS_PARALLELISM": "false",
