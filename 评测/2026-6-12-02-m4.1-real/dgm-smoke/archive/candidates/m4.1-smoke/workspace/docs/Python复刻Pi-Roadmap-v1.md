...
- **已完成（M4.1）**：eval 路径绝对化、validator pytest rootdir/test file 固定、过程产物 secret scan、full gate 入口。M4.0 回归 WARN 作为历史记录保留，M4.1 专门修复。
...
| **M4.1 Eval Hardening** | ✅ 已完成（2026-06-12） | 在 M4.0 基座上修复回归 WARN：`run_root` / workspace / summary / prompt / `EvalResult` 路径统一绝对化；validator pytest 显式 `--rootdir <workspace>` 并指定任务测试文件；新增 `scan_eval_artifacts_for_secrets`（扫描过程产物与 summary，默认忽略复制 workspace fixture）；`python -m mu.eval` 输出绝对 run dir、`Passed: x/y`、secret scan；新增 `python -m mu.eval_gate` 串联离线 pytest、真实 basic eval、DGM-lite fake-agent smoke 并写入 `评测/<date-run>/`。plan 见 `plan/M4.1-Eval-Hardening-plan.md`。 |
