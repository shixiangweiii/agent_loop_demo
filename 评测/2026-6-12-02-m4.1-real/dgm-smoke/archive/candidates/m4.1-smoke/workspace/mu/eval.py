...
def scan_eval_artifacts_for_secrets(
    ...
    *,
    ...
    files = [root_path] if root_path.is_file() else sorted(p for p in root_path.rglob("*") if p.is_file())
    for path in files:
...
def _secret_scan_ignore_reason(path: Path, root: Path) -> str | None:
    ...
    if "__pycache__" in parts or path.suffix in {".pyc", ".pyo"}:
        return "bytecode/cache artifact"
    return None
...
def _failure_stage(result: EvalResult) -> str:
    ...
    if result.agent_returncode not in (0, None):
        return "agent"
    if result.agent_returncode is None:
        return "agent"
    return "validator"
