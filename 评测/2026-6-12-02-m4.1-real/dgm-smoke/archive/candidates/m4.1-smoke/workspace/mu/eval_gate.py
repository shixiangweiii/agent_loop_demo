"""Full regression gate for M4.1 eval hardening.

This module intentionally keeps the gate boring and reproducible:
offline pytest, real-model basic eval, DGM-lite smoke, then one final
secret scan over the process artifacts.
"""
...
def default_gate_dir(now: datetime | None = None) -> Path:
    ...
    label = f"{current.year}-{current.month}-{current.day}-{current:%H%M%S}"
...
def run_full_gate(
    *,
    ...
    """Run the M4.1 full gate and write a compact report."""
    ...
    summary = {
        "name": "M4.1 Eval Hardening Full Gate",
...
def _run_dgm_smoke(gate_dir: Path, project: Path) -> dict[str, Any]:
    ...
    prompt_file.write_text("M4.1 smoke candidate: keep eval deterministic.\n", encoding="utf-8")
    ...
    try:
        entry = run_dgm_candidate(
            ...
            description="m4.1 full gate smoke",
            candidate_id="m4.1-smoke",
            project_root=project,
...
def _write_gate_summary(gate_dir: Path, summary: dict[str, Any], env: dict[str, str]) -> None:
    ...
    report_file = gate_dir / "m4.1-eval-hardening-report.md"
...
def _parse_args(argv: Sequence[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        ...
        description="Run the M4.1 eval hardening full gate",
    )
