...
Meta-tool 是持久化的 code-action 热路径：把常用工具流程写成 `.mu/metatools/*.json`，显式启用后注册为普通 tool。它运行任意 Python，安全等级等同 `code`，固定带 `code_exec` capability；`readonly/workspace` 下会整体拦截。
