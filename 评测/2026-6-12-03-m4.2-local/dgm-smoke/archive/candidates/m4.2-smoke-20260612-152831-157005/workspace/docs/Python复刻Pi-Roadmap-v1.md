...
| **M4.2 Procedural Memory / Meta-tools** | ✅ 已实现（2026-06-12） | 新增 `mu.metatool`：从 `.mu/metatools/*.json` / `MU_METATOOL_DIR` 加载 repo-local meta-tool spec，注册成普通工具，执行环境提供 `args` + `mu.read/write/edit/bash/call/log/result`；CLI 接 `--metatools` / `--metatool-dir` 与 `MU_METATOOLS` / `MU_METATOOL_DIR`；meta-tool 固定 `code_exec` capability，`readonly/workspace` 下整体拦截；DGM allowlist 增加 `.mu/metatools/*.json` 且 eval 环境指向候选 workspace；full gate 增加 meta-tool fake-model smoke。plan 见 `plan/M4.2-Procedural-Memory-Metatools-plan.md`。 |
