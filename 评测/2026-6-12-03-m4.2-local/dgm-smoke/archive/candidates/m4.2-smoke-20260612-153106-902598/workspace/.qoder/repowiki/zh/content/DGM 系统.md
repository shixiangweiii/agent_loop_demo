...
- [评测/2026-6-12-01/dgm-smoke/archive/archive.jsonl](file://评测/2026-6-12-01/dgm-smoke/archive/archive.jsonl)
</cite>

## 更新摘要
**所做更改**
- 新增 DGM-lite 基础设施的完整架构说明
- 更新候选隔离验证系统的实现细节
- 新增 append-only 归档机制的技术规范
- 完善路径范围限制和安全策略
- 增强评估套件集成的技术文档
- 补充实际操作示例和最佳实践
...
- 测试与示例
  ...
  - plan/M4.0-Eval-DGM-lite-plan.md：M4.0 计划文档，定义候选范围和归档规范
...
PLAN["M4.0-Eval-DGM-lite-plan.md<br/>计划规范"]
SMOKE["dgm-smoke 示例<br/>实际运行结果"]
end
DGM --> EVAL
DGM --> PERM
DGM --> EXTS
...
### 候选隔离验证系统
- 工作空间隔离
  - 每个候选在独立的复制工作空间中执行，确保与其他候选的完全隔离
  - 使用 copytree 进行项目复制，忽略特定目录和 .egg-info 文件
- 路径范围限制
  - 严格限制候选路径范围：.mu/extensions/*.py、.mu/prompts/*.{md,txt}、extensions/*.{py,md,txt}
  - 防止对 mu/ 核心文件、session、工具执行、TUI、权限层的直接修改
- 候选类型支持
  - 目录候选：直接覆盖允许范围内的文件
  - 补丁候选：应用只触碰允许路径的 patch
  - 生成式候选：在候选副本中让 μ 生成候选
- 安全验证
  - 在非 allow 权限下拒绝扩展候选执行
  - 对扩展路径进行额外校验，防止权限绕过
...
### Append-only 归档机制
- 归档格式
  - 使用 JSON Lines (JSONL) 格式存储，每行一条候选记录
  - 包含完整的元数据：ID、父级ID、描述、源类型、路径、变更路径、工作空间、评估结果等
- 最佳候选选择
  - 基于通过率、持续时间、token 消耗等指标自动选择最佳候选
  - 维护 latest-summary.json 和 latest-summary.md 文件
- 数据完整性
  - 所有敏感信息（API Key、Token、Secret）在写入前进行脱敏处理
  - 支持重新运行和历史回溯分析
...
- 示例路径参考
  ...
  - 实际运行示例可参考评测/2026-6-12-01/dgm-smoke
...
### 技术规范与标准
- 候选范围标准
  - .mu/extensions/*.py：扩展文件
  - .mu/prompts/*.{md,txt}：提示词文件
  - extensions/*.{py,md,txt}：扩展目录
- 归档格式规范
