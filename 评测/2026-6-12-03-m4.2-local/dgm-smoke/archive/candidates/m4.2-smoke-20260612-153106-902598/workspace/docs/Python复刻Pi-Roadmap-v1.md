...
| **M4.2 Procedural Memory / Meta-tools** | ✅ 已实现并本地验收（2026-06-12） | 新增 `mu.metatool`：从 `.mu/metatools/*.json` / `MU_METATOOL_DIR` 加载 repo-local meta-tool spec，注册成普通工具，执行环境提供 `args` + `mu.read/write/edit/bash/call/log/result`；CLI 接 `--metatools` / `--metatool-dir` 与 `MU_METATOOLS` / `MU_METATOOL_DIR`；meta-tool 固定 `code_exec` capability，`readonly/workspace` 下整体拦截；DGM allowlist 增加 `.mu/metatools/*.json` 且 eval 环境指向候选 workspace；full gate 增加 meta-tool fake-model smoke。验收结果：全量离线 `103 passed, 1 skipped`；离线 full gate 见 `评测/2026-6-12-03-m4.2-local/`，overall PASS、secret scan PASS、DGM smoke PASS、meta-tool smoke PASS；真实模型 gate 因当前 shell 未提供模型环境变量未运行。plan 见 `plan/M4.2-Procedural-Memory-Metatools-plan.md`。 |
