...
- **后续范围**：🟠 投机/异步执行（隐藏工具等待）；自动应用通过候选；prompt 记忆 / meta-tool 生成与分发。
- **完成标志**：每项可独立开关，关掉后退回 v1 形态；M4.2 已具备 eval 护栏、候选 archive 和显式程序性记忆，后续能力必须先过 eval / full gate。
...
| **v2（M4 后续，研究级）** | ⏳ 未开始 | 投机/异步执行、自动应用通过候选、prompt 记忆 / meta-tool 自动生成与分发等高不确定能力。 |
