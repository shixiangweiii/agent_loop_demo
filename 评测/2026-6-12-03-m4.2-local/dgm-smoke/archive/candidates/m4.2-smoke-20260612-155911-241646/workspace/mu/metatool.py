...
_DEFAULT_META_TOOL_TIMEOUT = 300.0
...
def metatool_timeout_seconds() -> float:
    raw = os.environ.get("MU_METATOOL_TIMEOUT")
    if not raw:
        return _DEFAULT_META_TOOL_TIMEOUT
    try:
        timeout = float(raw)
    except ValueError:
        return _DEFAULT_META_TOOL_TIMEOUT
    ...
    async def _execute(self, spec: MetaToolSpec, args: dict[str, Any]) -> ToolResult:
        ...
        try:
            return await asyncio.wait_for(
                ...
                timeout=metatool_timeout_seconds(),
            )
