...
### M4 — 研究级扩展（v2，M4.3 受控 promotion 已落地）
...
- **后续范围**：🟠 投机/异步执行（隐藏工具等待）；prompt 记忆 / meta-tool 自动生成与分发；更自动化的候选生成策略。
- **完成标志**：每项可独立开关，关掉后退回 v1 形态；M4.3 已具备 eval 护栏、候选 archive、显式程序性记忆和受控自改闭环，后续能力必须先过 eval / full gate。
...
| **DGM-lite：扩展提案经 eval 护栏自改进** | 🟡 v2 | **M4.3 ✅ 受控 promotion 已落地** | 已落 eval runner + 候选隔离 + append-only archive + full gate + patch-first 显式 apply；更自动化生成留后续 | Darwin Gödel Machine + 程序性记忆 |
...
| **M4.3 Guarded DGM Promotion** | ✅ 已实现（2026-06-12） | 新增 `mu.dgm_promote`：从 DGM archive 选择 best/指定候选，要求候选 eval 全通过且 secret scan PASS，默认生成 `promotion.patch` + JSON/MD summary，不改主仓库；显式 `--apply` 时要求 git worktree、目标 changed_paths 无本地改动、sandbox `pytest -q` preflight PASS，并写回前备份目标路径；promotion 产物继续 secret scan，命中真实 env secret 或 `sk-...` 模式拒绝 apply。full gate 增加 `dgm_promotion_smoke`。plan 见 `plan/M4.3-Guarded-DGM-Promotion-plan.md`。 |
| **v2（M4 后续，研究级）** | ⏳ 未开始 | 投机/异步执行、prompt 记忆 / meta-tool 自动生成与分发、更自动化候选生成策略等高不确定能力。 |
