...
candidates are not applied during evaluation; guarded promotion lives in
`mu.dgm_promote`.
"""
...
GenerateCommandBuilder = Callable[[Path, str], list[str]]
CandidateScope = Literal["dgm", "prompt"]
...
def run_dgm_candidate(
    *,
    ...
    candidate_scope: CandidateScope = "dgm",
) -> DgmArchiveEntry:
    ...
    if permission != "allow" and any(_requires_exec_permission(p) for p in changed_paths):
        raise DgmConfigError(
            f"extension/metatool candidates cannot run under permission={permission}; "
    ...
    run = run_eval_suite(
        ...
        extra_env={
            ...
            "MU_METATOOL_DIR": str(candidate_root / ".mu" / "metatools"),
        },
...
def _apply_candidate(
    ...
    *,
    ...
    if source_type == "dir":
        ...
        return _overlay_candidate_dir(candidate_root, source_path, candidate_scope)
    if source_type == "patch":
        if source_path is None:
            raise DgmConfigError("patch source requires source_path")
        return _apply_patch_candidate(candidate_root, source_path, candidate_scope)
    if source_type == "generate":
        ...
        return _generate_candidate(
            ...
            generate_cmd_builder, candidate_scope,
        )
...
def _overlay_candidate_dir(
    ...
    for p in sorted(source_dir.rglob("*")):
        if not p.is_file():
            continue
        rel = _validate_candidate_path(p.relative_to(source_dir), candidate_scope)
...
def _apply_patch_candidate(
    ...
    changed = sorted(
        {_validate_candidate_path(p, candidate_scope) for p in _paths_from_patch(patch_text)}
    )
    if not changed:
...
def _generate_candidate(
    ...
    if candidate_scope == "prompt":
        task = (
            "You are generating an auto DGM prompt-snippet candidate. Only create or edit files "
            "under .mu/prompts with .md or .txt suffixes. Do not edit core code, tests, "
            "extensions, meta-tools, or documentation outside .mu/prompts.\n\n"
            + prompt
        )
    else:
        task = (
            "You are generating a DGM-lite candidate. Only create or edit files under "
            ".mu/extensions, .mu/prompts, .mu/metatools, or extensions. Do not edit mu core files.\n\n"
            + prompt
        )
    cmd = (
        generate_cmd_builder(candidate_root, task)
        if generate_cmd_builder is not None
        else [sys.executable, "-m", "mu", task]
    )
    completed = subprocess.run(
        cmd,
        cwd=candidate_root,
    ...
    return [_validate_candidate_path(p, candidate_scope) for p in changed]
...
def _validate_candidate_path(
    ...
    if candidate_scope == "prompt":
        if rel.startswith(".mu/prompts/") and suffix in _PROMPT_SUFFIXES:
            return rel
        raise DgmConfigError(
            f"candidate path is outside the prompt-only DGM scope: {rel}; allowed: "
            ".mu/prompts/*.{md,txt}"
        )
    ...
    if rel.startswith(".mu/prompts/") and suffix in _PROMPT_SUFFIXES:
        return rel
    if rel.startswith(".mu/metatools/") and suffix == ".json":
        return rel
    if rel.startswith("extensions/") and suffix in {".py", ".md", ".txt"}:
        return rel
    raise DgmConfigError(
        f"candidate path is outside the DGM-lite scope: {rel}; allowed: "
        ".mu/extensions/*.py, .mu/prompts/*.{md,txt}, .mu/metatools/*.json, extensions/*"
    )


def _requires_exec_permission(rel: str) -> bool:
    return rel.startswith(".mu/extensions/") or rel.startswith(".mu/metatools/") or (
