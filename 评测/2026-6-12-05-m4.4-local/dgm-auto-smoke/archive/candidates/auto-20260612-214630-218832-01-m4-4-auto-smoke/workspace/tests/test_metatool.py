...
async def test_metatool_soft_timeout_cancels_late_tool_calls(monkeypatch, tmp_path):
    monkeypatch.setenv("MU_METATOOL_TIMEOUT", "0.01")
    ...
    out = await reg.execute("slow_write", {})
    await asyncio.sleep(0.15)

    assert "soft timeout" in out
    assert not target.exists()
