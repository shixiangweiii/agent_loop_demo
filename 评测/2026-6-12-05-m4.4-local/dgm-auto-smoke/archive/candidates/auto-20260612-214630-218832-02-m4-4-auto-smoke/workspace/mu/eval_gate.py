"""Full regression gate for M4.4 auto DGM candidate generation.

This module intentionally keeps the gate boring and reproducible:
offline pytest, real-model basic eval, DGM-lite smoke, DGM auto smoke,
meta-tool smoke, DGM promotion smoke, then one final secret scan over the
process artifacts.
"""
...
from .dgm_auto import run_dgm_auto
...
def run_full_gate(
    *,
    ...
    """Run the M4.4 full gate and write a compact report."""
...
def _run_dgm_auto_smoke(gate_dir: Path, project: Path) -> dict[str, Any]:
    smoke_dir = gate_dir / "dgm-auto-smoke"
    if smoke_dir.exists():
        shutil.rmtree(smoke_dir)
    archive = smoke_dir / "archive"
    fake_agent = smoke_dir / "fake_agent.py"
    fake_generator = smoke_dir / "fake_generator.py"
    fake_agent.parent.mkdir(parents=True, exist_ok=True)
    fake_agent.write_text(_fake_agent_source(), encoding="utf-8")
    ...
    def build_agent(_workspace: Path, prompt: str) -> list[str]:
        return [_preferred_python(project), str(fake_agent), prompt]

    def build_generator(_workspace: Path, prompt: str) -> list[str]:
        return [_preferred_python(project), str(fake_generator), prompt]
    ...
    try:
        run = run_dgm_auto(
            description="m4.4 auto smoke",
            count=2,
            project_root=project,
            archive_dir=archive,
            suite=basic_coding_suite(timeout_seconds=120),
            agent_cmd_builder=build_agent,
            generate_cmd_builder=build_generator,
            env=fake_env,
            require_model_env=True,
        )
    except DgmConfigError as e:
        return {
            "name": "dgm_auto_fake_agent_smoke",
            "passed": False,
            "error": str(e),
        }
    archive_jsonl = archive / "archive.jsonl"
    ...
    return {
        "name": "dgm_auto_fake_agent_smoke",
        "passed": (
            run.generated_count == 2
            and run.passed_count == 2
            and bool(run.best_candidate_id)
            and archive_jsonl.exists()
            and len(archive_jsonl.read_text(encoding="utf-8").splitlines()) == 2
            and latest_summary.exists()
            and Path(run.summary_json_file).exists()
            and run.secret_scan_passed
        ),
        "run_id": run.id,
...
def _write_gate_summary(gate_dir: Path, summary: dict[str, Any], env: dict[str, str]) -> None:
    ...
    report_file = gate_dir / "m4.4-auto-dgm-candidate-generation-report.md"
...
def _check_detail(check: dict[str, Any]) -> str:
    ...
    if check.get("name") == "dgm_auto_fake_agent_smoke":
        if check.get("error"):
            return str(check["error"])
        return (
            f"generated={check.get('generated_count')}; "
            f"passed={check.get('passed_count')}; "
            f"best=`{check.get('best_candidate_id')}`; "
            f"summary=`{check.get('summary_json_file')}`"
        )
...
def _parse_args(argv: Sequence[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        ...
        description="Run the M4.4 auto DGM candidate generation full gate",
    )
