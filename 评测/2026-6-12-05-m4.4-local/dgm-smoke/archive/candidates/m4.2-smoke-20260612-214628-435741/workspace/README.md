# μ (mu) — minimal Pi-style coding agent · M4.4 auto DGM candidate generation
...
- **M4.2** 程序性记忆 / meta-tools：repo-local JSON meta-tool，把已验证高频流程注册成普通工具；默认关，显式 `--metatools` 开。
- **M4.3** 受控 DGM promotion：通过候选默认只生成 patch/report；显式 `--apply` 才经脏路径检查 + sandbox pytest 后写回。
- **M4.4** 自动 DGM 候选生成：自动生成 prompt-snippet candidates，逐个隔离评测并写入 archive；promotion 仍需人工显式 `dgm_promote --apply`。
...
## Meta-tools 程序性记忆（M4.2，默认关）

Meta-tool 是持久化的 code-action 热路径：把常用工具流程写成 `.mu/metatools/*.json`，显式启用后注册为普通 tool。它运行任意 Python，安全等级等同 `code`，固定带 `code_exec` capability；`readonly/workspace` 下会整体拦截。只在可信仓库中启用 `--metatools`，因为磁盘上的 spec code 会在调用时进程内执行。

```bash
./.venv/bin/python -m mu --metatools "列出可用 meta-tools"
./.venv/bin/python -m mu --metatools --metatool-dir ./.mu/metatools "调用 quick_pytest"
```
...
```json
{
  "name": "quick_pytest",
  "version": "0.1",
  "description": "Run pytest quickly for the current repo or target path.",
  "parameters": {
    "type": "object",
    "properties": {
      "target": {"type": "string"}
  ...
  "code": "import shlex\ntarget = shlex.quote(args.get('target') or '.')\nout = mu.bash(f'./.venv/bin/python -m pytest -q {target}', timeout=180)\nmu.result(out)"
}
```

- `MU_METATOOLS=1` 等价 `--metatools`；`MU_METATOOL_DIR` 覆盖默认目录。
- 外层 meta-tool soft timeout 默认 300s，可用 `MU_METATOOL_TIMEOUT` 覆盖；内层 `mu.bash(..., timeout=...)` 应小于外层上限。
- `list_metatools` 列出已加载工具和坏 spec 错误；`reload_metatools` 重新扫描目录。
- DGM-lite candidate 可包含 `.mu/metatools/*.json`，通过项仍只归档，不自动应用回主仓库。

## Eval / DGM-lite / Gate（M4.4）
...
`python -m mu.dgm` 在复制出来的候选 workspace 中叠加扩展/提示词/meta-tool 候选，跑同一 eval suite，并写入 append-only archive；通过项不会自动应用回主仓库。
...
`python -m mu.dgm_auto` 自动生成一批 prompt-snippet candidates，再逐个走同一 eval/archive 链路。默认只允许生成 `.mu/prompts/*.md|*.txt`，不会自动应用主仓库变更。

```bash
./.venv/bin/python -m mu.dgm_auto --archive-dir dgm_archive --count 3 --description "improve basic coding eval behavior"
```

M4.2/M4.3 候选范围刻意很窄：`.mu/extensions/*.py`、`.mu/prompts/*.{md,txt}`、`.mu/metatools/*.json`、`extensions/*.{py,md,txt}`。M4.4 自动候选更窄：只生成 `.mu/prompts/*.{md,txt}`。`readonly/workspace` 下扩展 / meta-tool 候选会被拒绝，避免绕过 M3.5 权限边界。

`python -m mu.dgm_promote` 把已通过 archive candidate 转成可审计 promotion。默认只生成 patch/report，不改主仓库；显式 `--apply` 才要求 git worktree、目标路径无本地改动、sandbox `pytest -q` 通过，然后备份并写回 changed paths。

```bash
./.venv/bin/python -m mu.dgm_promote --archive-dir dgm_archive
./.venv/bin/python -m mu.dgm_promote --archive-dir dgm_archive --candidate best --apply
```

M4.4 full gate 串起离线 pytest、真实模型 basic eval、DGM-lite fake-agent smoke、DGM auto fake-agent smoke、meta-tool fake-model smoke、DGM promotion smoke，并在最终产物上再跑一次 secret scan。真实模型 eval 只读取当前 shell 的 `MU_BASE_URL` / `MU_MODEL` / `MU_API_KEY` 或 `OPENAI_API_KEY`，不会写入文件。

```bash
./.venv/bin/python -m mu.eval_gate --run-root "评测/2026-6-12-05"
...
M4.2 已做：repo-local meta-tools 程序性记忆（默认关、显式加载、受 permission gate 约束）与 full gate meta-tool smoke。
M4.3 已做：通过 DGM candidate 的 guarded promotion（默认 patch-first，显式 apply，路径级脏检查 + sandbox pytest）。
M4.4 已做：自动生成 prompt-snippet DGM candidates，逐个隔离评测并写入 archive；自动生成不等于自动应用。
留作 v2 后续：prompt 记忆、meta-tool 自动生成/分发、投机/异步执行。其余不做：受限解释器、E2B/Modal 具体实现、Web UI / textual serve、完整 compaction、$ 精确计费、并行工具、多 provider 切换。
