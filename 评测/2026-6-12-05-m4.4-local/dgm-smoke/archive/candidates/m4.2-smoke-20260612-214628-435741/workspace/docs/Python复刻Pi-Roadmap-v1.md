...
- **已完成（M4.2）**：repo-local meta-tools 程序性记忆：显式 `--metatools` 加载 `.mu/metatools/*.json`，把高频 code-action 流程注册成普通工具，固定走 `code_exec` permission gate；DGM allowlist 支持 meta-tool candidate，但仍只归档不自动应用。
- **已完成（M4.3）**：受控 DGM promotion：通过候选默认只生成 patch/report；显式 `--apply` 才经路径级脏检查、sandbox pytest、备份后写回主仓库，不自动 commit/stage/push。
- **已完成（M4.4）**：自动 DGM 候选生成：串行生成 prompt-snippet candidates，逐个隔离评测并写入 archive / auto-run summary；仍不自动 apply / commit / stage / push。
- **后续范围**：🟠 投机/异步执行（隐藏工具等待）；prompt 记忆 / meta-tool 自动生成与分发。
- **完成标志**：每项可独立开关，关掉后退回 v1 形态；M4.4 已具备 eval 护栏、候选 archive、显式程序性记忆、受控自改闭环和自动 prompt 候选生成，后续能力必须先过 eval / full gate。
...
| **DGM-lite：扩展提案经 eval 护栏自改进** | 🟡 v2 | **M4.4 ✅ 自动候选生成已落地** | 已落 eval runner + 候选隔离 + append-only archive + full gate + patch-first 显式 apply + 自动 prompt 候选生成；自动应用仍不做 | Darwin Gödel Machine + 程序性记忆 |
| **程序性记忆 / meta-tool 编译热路径** | 🟡 v2 | **M4.2 ✅ repo-local meta-tools** | 把高频 code-action 流程固化为显式加载的确定性工具；自动学习/生成留后续 | ProcMEM / AWM / meta-tools |
...
| **M4.2 Procedural Memory / Meta-tools** | ✅ 已实现并本地验收（2026-06-12） | 新增 `mu.metatool`：从 `.mu/metatools/*.json` / `MU_METATOOL_DIR` 加载 repo-local meta-tool spec，注册成普通工具，执行环境提供 `args` + `mu.read/write/edit/bash/call/log/result`；CLI 接 `--metatools` / `--metatool-dir` 与 `MU_METATOOLS` / `MU_METATOOL_DIR`；meta-tool 固定 `code_exec` capability，`readonly/workspace` 下整体拦截；DGM allowlist 增加 `.mu/metatools/*.json` 且 eval 环境指向候选 workspace；full gate 增加 meta-tool fake-model smoke。二次复核修复：外层 timeout 默认 300s + `MU_METATOOL_TIMEOUT` 覆盖，README 示例改为 shell quoting，补 soft-timeout/cancel 测试。验收结果：全量离线 `105 passed, 1 skipped`；离线 full gate 见 `评测/2026-6-12-03-m4.2-local/`，overall PASS、secret scan PASS、DGM smoke PASS、meta-tool smoke PASS；真实模型 gate 因当前 shell 未提供模型环境变量未运行。plan 见 `plan/M4.2-Procedural-Memory-Metatools-plan.md`。 |
| **M4.3 Guarded DGM Promotion** | ✅ 已实现并本地验收（2026-06-12） | 新增 `mu.dgm_promote`：从 DGM archive 选择 best/指定候选，要求候选 eval 全通过且 secret scan PASS，默认生成 `promotion.patch` + JSON/MD summary，不改主仓库；显式 `--apply` 时要求 git worktree、目标 changed_paths 无本地改动、sandbox `pytest -q` preflight PASS，并写回前备份目标路径；promotion 产物继续 secret scan，命中真实 env secret 或 `sk-...` 模式拒绝 apply。full gate 增加 `dgm_promotion_smoke`。验收结果：全量离线 `116 passed, 1 skipped`；离线 full gate 见 `评测/2026-6-12-04-m4.3-local/`，overall PASS、secret scan PASS、DGM smoke PASS、meta-tool smoke PASS、promotion smoke PASS；真实模型 gate 因当前 shell 未提供模型环境变量未运行。plan 见 `plan/M4.3-Guarded-DGM-Promotion-plan.md`。 |
| **M4.4 Auto DGM Candidate Generation** | ✅ 已实现并本地验收（2026-06-12） | 新增 `mu.dgm_auto`：串行生成多个 prompt-snippet candidates（默认 3 个），每个候选调用现有 DGM generate→eval→archive 链路；自动候选 scope 收窄到 `.mu/prompts/*.{md,txt}`，失败候选记录在 auto-run summary 后继续；summary 写入 `dgm_archive/auto-runs/<run_id>/summary.{json,md}`，不改变 archive JSONL schema。full gate 增加 `dgm_auto_fake_agent_smoke`；自动生成不等于自动应用，promotion 仍走 `mu.dgm_promote --apply`。plan 见 `plan/M4.4-Auto-DGM-Candidate-Generation-plan.md`。 |
| **v2（M4 后续，研究级）** | ⏳ 未开始 | 投机/异步执行、prompt 记忆 / meta-tool 自动生成与分发等高不确定能力。 |
